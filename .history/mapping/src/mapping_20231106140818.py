...
from mapping.src.criterion import Criterion
from mapping.src.frame import RGBDFrame
from mapping.src.functions.initialize_sdf import initemb_sdf
from mapping.src.functions.render_helpers import bundle_adjust_frames
from mapping.src.functions.render_helpers import fill_in, render_rays
from mapping.src.loggers import BasicLogger
from mapping.src.utils.import_util import get_decoder, get_property
from mapping.src.utils.keyframe_util import multiple_max_set_coverage
from mapping.src.utils.mesh_util import MeshExtractor
...
class Mapping:
    ...
    def run(self, first_frame, update_pose):
        ...
        if self.run_ros and False:
            ...
            self.do_mapping(tracked_frame=first_frame, update_pose=update_pose,init=True)
            ...
            for frame_id in progress_bar:
                ...
                self.writer.add_scalar('mapping/time', map_time, frame_id)
                print("mapping time: ", map_time)
                
                if frame_id%1000==0:
                    mesh = self.extract_mesh(res=self.mesh_res, clean_mesh=False, map_states=self.map_states)
                    self.logger.log_mesh(mesh,frame_id)
                    print("Mesh saved for: ",frame_id)
    ...
    def do_mapping(self, tracked_frame=None, update_pose=True, init=False):
        self.decoder.train()
    ...
    @torch.no_grad()
    def extract_mesh(self, res=1, clean_mesh=False, map_states=None):
